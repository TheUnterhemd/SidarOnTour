...
console.log(
  "Bepackt mit seiner Tasche in der Rechten und seiner Laptoptasche in der Linken, betritt Sidar die große Halle des Leipziger Hauptbahnhofes. Geschäftiges treiben, der typische Feierabendverkehr, voll mit Pendlern. Das Echo der hohen Halle verstärkt noch zusätzlich den Geräuschpegel der Leute die man am liebsten ignorieren würde.\n"
);

console.log(
  "Fokussiert so schnell wie möglich zu seinem Gleis zu kommen, bemerkt er plötzlich ein Ziehen an seinem Arm.\n"
);

console.log(
  "Sidar beäugt erstaunt die heruntergekommene Person, die seinen Arm festhält, ihm die andere geöffnete Hand hinstreckt und etwas nicht erkennbares nuschelt.\n"
);

console.log(`"Wehmtzhmrrrr….”.\n`);

pos = [
  "dem Dude einen 5 Euro Schein in die Hand drücken",
  "sich von dem Dude lösen und so schnell es geht zum Gleis gehen",
];
let answer = readlineSync.keyInSelect(
  pos,
  "Sidar räuspert sich, was soll er tun?"
);
console.clear();
if (answer === 0) {
...
console.log(
  "Der typisch feine Zuggeruch – eine Mischung aus Parfüm, Bier und Scheiße erfreut direkt Sidars Nase, sein Blick schweift über die Sitplätze:"
);
...
console.log(
  "Zu seiner rechten drei besoffene Fußballfans mit Glatze die lautstark Fußballieder gröhlen.\n"
);
...
if (sitz === 0) {
  ...
  console.log(
    "Der junge steckt seinen kleinen Finger tief in die Nase und lässt einen saftig schleimigen Popel zum Vorschein kommen."
  );
  ...
  if (share === 0) {
    if (sidar.presents.length < 1) {
      ...
      let kidPresent = readlineSync.keyInSelect(
        sidar.presents,
        "was soll Sidar dem Kind anbieten"
      );
      console.log(
        `Das Kind reibt den Popel in den Nachbarsitz und fängt an den ${sidar.presents[kidPresent]} zu lutschen. (Es sieht zufrieden aus)`
      );
    ...
    if (firstDice === 0) {
      ...
      console.log(
        `Das ${popel.type} hat einen saftigen grünen Popel der Stärke ${popel.dice} gefunden.`
      );
      sidar.dice = dices();
      console.log(
        `Sidar wühlt hecktisch in seinen Taschen und findet ein altes Taschentuch der Stärke ${sidar.dice}`
      );
      if (sidar.dice > popel.dice) {
        console.log(
          "Zum Glück benötigt Sidar sein Tachentuch nicht, da Die Mutter das ganze noch rechtzeitig mitbekommt und die Hand des Jungen festhält."
        );
        console.log(
          "Nach einer gehörigen Standpauke entschuldigt sich die Mutter und schenkt ihm einen Tampon."
        );
...
//Glatzen
  ...
  console.log(
    "Ganz tief im inneren hofft Sidar einfach nur nicht angesprochen zu werden, doch sein Gegenüber reißt von jetzt auf gleich die Augen weit auf und lehnt sich immer weiter Richtung Sidar."
  );
  let hool = ["ja", "nein"];
  let help = readlineSync.keyInSelect(
    hool,
    "Soll Sidar ihm etwas anbieten um ihm zu helfen?"
  );
  console.clear();
  if (help === 0) {
    if (sidar.presents.length < 1) {
      ...
      let hoolHelp = readlineSync.keyInSelect(
        sidar.presents,
        "was soll Sidar der Fleischkappe anbieten"
      );
      console.log(
    ...
    if (secondDice === 0) {
      ...
      sidar.dice = dices();
      console.log(
        `Sidar beugt sich hecktisch nach Rechts und Links, seine Ausweichtaktik hat eine Stärke von ${sidar.dice}`
      );
      if (sidar.dice > womit.dice) {
        console.log(
          "Glück gehabt, der Hool presst mit vollem Einsatz einen Ohrenbetäubenden Rülpser raus. Mit einem: “Lmentschullije” drückt der Hool Sidar ein Bier in die Hand."
        );
        sidar.presents.push("Bier");
      } else if (sidar.dice < womit.dice) {
        console.log(
          "Sidars wackeln bringt nichts und der Hool kotzt mit einer formgleichen Fontäne komplett auf Sidars Schoß. Nun ist Sidar eins mit dem stinkenden Zug."
        );
...
//Haltestelle 3
console.log("Der nächste rege Wechsel an der Haltestelle.");
console.log(
  ...
  if (pornoShare === 0) {
    if (sidar.presents.length < 1) {
      ...
      if (sidar.presents[pornoPresent] === "OnlyFans Abo") {
        console.log(
          "Der creepy Typ macht Auge, leichtes sabbern glitzert aus seinem Mundwinkel. Rasch schnürt er seinen Mantelürtel wieder fester, fummelt hektisch sein Smartphone heraus und begibt sich schnell zur Zugtoilette, auf welcher man die nächsten 30 Minuten nur noch seltsame Geräusche vernehmen kann."
        );
        sidar.presents.splice(pornoPresent, 1);
      } else {
        let noPresent = dices();
        console.log(
          `"Mr. Ich habe nichts drunter" ist wenig beeindruckt und schmeißt dein ${sidar.presents[pornoPresent]} weg. Er fackelt auch nicht lange und öffnet seinen Mantel damit sich Anblick in Sidars Hirn einbrennt. Die Attecke des Pornobalkenträgers ekelt Sidar mit ${noPresent}.`
        );
        sidar.presents.splice(pornoPresent, 1);
      }
    }
...
pos5 = [
  "Nein, du bist immer Wunderschön, aber in der Jacke besonders!",
  "Im Vergleich zu allen anderen wirst du immer die Schönste sein!",
];
...
pos6 = [
  "“Nein Liebste, du bist schön von innen wie auch außen!”",
  "Sidar fällt keine Antwort ein und seine Freundin wird grimmiger.",
];
let beautiful = readlineSync.keyInSelect(
  pos6,
  "“Also bin ich es dir nur Wert wenn ich mich hübsch mache?”."
);
console.clear();
if (beautiful === 0) {
  let right = dices();
  ...
  let wrong = dices();
  console.log(
    `Ohhhh, Sidar hätte sich lieber etwas einfallen lassen sollen. Sein Puls steigt um weitere ${wrong}.`
  );
  sidar.excitement += wrong;
  console.log(
    `Sidars Puls klettert auf Sage und Schreibe ${sidar.excitement}.`
  );
} else {
  console.log("Sidar ist so genervt und springt weinend in den Zug");
  console.log(`  _____                 ____              \r\n \/ ___\/__ ___ _  ___   \/ __ \\_  _____ ____\r\n\/ (_ \/ _ \`\ / \' \\\/ -_) \/ \/_\/ \/ |\/ \/ -_) __\/\r\n\\___\/\\_,_\/_\/_\/_\/\\__\/  \\____\/|___\/\\__\/_\/   \r\n                                          
...
`);
  ...
  let present1 = readlineSync.keyInSelect(
    sidar.presents,
